...
public class Libro extends Producto {
	 ...
	 * 				Excepción di el id no es válido.
	 ...
	 */
	private boolean esValidoAutor(String autor) {
	 ...
	 */
	protected GeneroEditorial getGenero() {
		return genero;
	}
	...
	protected void setBestseller(boolean bestseller) {
	...
	public float getPrecio(int dias, TipoAlquiler tipo) {
		...
		if (tipo == TipoAlquiler.NORMAL) {
			...
			return total;
		} else if (tipo == TipoAlquiler.BESTSELLER) {
			total = tipo.getPrecio();
			if (dias > 2)
				total += tipo.getPrecio() * 0.2 * (dias - 2);
			return total;
